...
const projects = [
  ...
  },
  {
    id: "youiux",
    name: "YouiUX",
    description: "YouiUX is a next-generation design tool built for the web3 era, powered by AI to enhance your workflow and reimagine your creative process.",
    image: "/project-youiux.svg",
          ...
          {projects.map(project => (
            <div key={project.id} className="kitchen-card p-6 spotlight">
              ...
              <button 
                ...
                onClick={project.id === "chat" || project.id === "coodapp" || project.url ? 
                  () => handleNavigateToApp(project.id) : 
                  () => setActiveProject(project.id)}
              >
                {project.url ? `Visit ${project.name} →` : 
                 (project.id === "chat" || project.id === "coodapp") ? `Open ${project.name} →` : 
                 "View Details →"}
              </button>
            </div>
          ))}
